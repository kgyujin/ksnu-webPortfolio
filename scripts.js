document.addEventListener('DOMContentLoaded', function() {
    ...
    const projects = {
        ...
        project2: {
            ...
            issues: [
                ...
                {
                    title: "API 전송 에러",
                    description: "카카오톡 메시지 전송 시, 데이터베이스에서 사용자의 이름이 포함되지 않고 JSON 형식으로 데이터가 전송되는 오류가 발생했습니다. 이를 해결하기 위해 추가적인 테스트와 개발 지원을 진행했습니다."
                }
            ],
    ...
    function openModal(project) {
        projectDetail.innerHTML = `
            ...
            <h3>이슈</h3>
            <ul>
            ${project.issues ? project.issues.map(issue => `<li><strong>${issue.title}:</strong> ${issue.description}</li>`).join('') : ''}
            </ul>
            <h3>리뷰</h3>
            <p>${project.review}</p>
            <hr>
            <img src="${project.image}" alt="${project.title} 이미지" style="max-width: 60%; height: auto;">
        `;
